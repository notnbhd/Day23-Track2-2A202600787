...
smoke: ## health-check all 7 services
	...
	@curl -fsS http://localhost:8010/healthz   > /dev/null && echo "  app:           OK"
	...
	@curl -fsS http://localhost:3010/api/health | grep -q '"database":"ok"' && echo "  grafana:       OK"
...
load: ## run baseline locust load (concurrency=10, 60s)
	cd 02-prometheus-grafana/load-test && \
	  locust -f locustfile.py --headless -u 10 -r 2 -t 60s --host http://localhost:8010
...
trace: ## generate one traced request and print its trace_id
	@curl -sS -X POST http://localhost:8010/predict \
